...
#[test]
fn history_reread_while_waiting_on_ask_user_keeps_pending_question_pinned() {
    // AskUserQuestion pinning depends on the pending request surviving a
    // provider history re-read while the turn is still waiting on the user.
    // Codex reservation/hydrate work must not change that contract.
    let user_item_id = "user:ask-pin-1111-4222-8333-444455556666";
    let mut relay = test_state();
    relay.activate_thread(
        test_thread("thread-ask-pin", "/tmp/project"),
        "/tmp/project",
        DEFAULT_MODEL,
        DEFAULT_APPROVAL_POLICY,
        DEFAULT_SANDBOX,
    ...
    relay.upsert_user_message(
        user_item_id.to_string(),
        "what should I name this?".to_string(),
        "claude-turn-ask".to_string(),
    );
    relay.add_pending_ask_user_question(test_pending_ask_user_question("thread-ask-pin"));
    relay.set_thread_status(
        "thread-ask-pin",
        "active".to_string(),
        vec!["waitingOnAskUser".to_string()],
    ...
    relay.load_thread_data(
        ThreadSyncData {
            thread: test_thread("thread-ask-pin", "/tmp/project"),
            status: "active".to_string(),
            active_flags: vec!["waitingOnAskUser".to_string()],
            transcript: vec![TranscriptEntryView {
                item_id: Some(user_item_id.to_string()),
                ...
                text: Some("what should I name this?".to_string()),
                status: "completed".to_string(),
                turn_id: Some("claude-turn-ask".to_string()),
                tool: None,
                content_state: crate::protocol::TranscriptContentState::Full,
            }],
        },
    ...
    assert!(
        relay.pending_ask_user_questions.contains_key("ask:1"),
        "history re-read must keep the pending AskUserQuestion so the UI pin remains"
    );
    assert!(
        relay
            .active_flags
        ...
        "waitingOnAskUser flag must survive history re-read for pin UX"
    );
    let user_messages = relay
        .transcript
        .iter()
