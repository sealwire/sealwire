...
// True when any review job is still running. Uses the job *status* rather than a
// non-empty check, because terminal jobs linger in `active_review_jobs` for a
// retention window so the UI can flash their final state.
...
// True when a review is stuck in the persistent Blocked state — the workspace
// stays locked until the user resolves it (stops the reviewer).
