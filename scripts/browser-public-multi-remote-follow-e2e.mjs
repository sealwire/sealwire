...
async function main() {
  ...
  try {
    ...
    assert.ok(
      pairingUrl.startsWith(`${remoteOrigin}/?pairing=`),
    ...
    await waitForPairedRemote(remotePageA);
    ...
    await waitForPairedRemote(remotePageB);
    ...
    await dumpBrowserState(localPage, remotePageA, remotePageB);
    throw error;
  } finally {
...
function attachPageDebugLogging(page, label) {
  page.on("console", (message) => {
    ...
    console.log(`[public-multi-remote-follow-e2e:${label}:console:${message.type()}] ${text}`);
  });
  page.on("pageerror", (error) => {
    console.error(
      `[public-multi-remote-follow-e2e:${label}:pageerror] ${error.stack || error.message}`
    );
  });
  page.on("requestfailed", (request) => {
    const failure = request.failure();
    console.error(
      `[public-multi-remote-follow-e2e:${label}:requestfailed] ${request.method()} ${request.url()} ${failure?.errorText || ""}`.trim()
    );
  });
}
...
async function dumpBrowserState(localPage, remotePageA, remotePageB) {
  if (localPage) {
    console.error("\n[local page]");
  ...
  if (remotePageA) {
    console.error("\n[remote page A]");
    console.error(await safeText(remotePageA, "#remote-client-log"));
  }
  if (remotePageB) {
    console.error("\n[remote page B]");
    console.error(await safeText(remotePageB, "#remote-client-log"));
  }
}
...
async function waitForBrokerConnection(url, timeoutMs = 30000) {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (session?.broker_connected) {
        return;
      }
    } catch {}
    await delay(300);
  }
  throw new Error(`timed out waiting for broker connection: ${url}`);
}
...
function resolvePrivateIpv4() {
  ...
  throw new Error("unable to determine private IPv4 address for browser e2e");
}
